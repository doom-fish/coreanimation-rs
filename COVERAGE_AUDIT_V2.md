...
Audit methodology: Enumeration of QuartzCore `CA*.h` headers (27 files) filtered to macOS-available symbols only. Cross-referenced against swift-bridge/ thunk layer and src/ Rust wrappers. The v1 audit identified 194 public macOS CoreAnimation symbols; v2 confirms the SDK surface is unchanged in 26.2.sdk and the crate still wraps 193 symbols (99.5% coverage). One symbol (`CAOpenGLLayer`) remains EXEMPT due to OpenGL deprecation on macOS.
