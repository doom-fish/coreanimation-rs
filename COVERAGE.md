...
`coreanimation-rs` keeps the original broad modules (`animation`, `layer`, `display_link`, `emitter`, `renderer`, and `transaction`) and layers `ca_*` extensions on top.

What the numbers measure: `COVERAGE_AUDIT.md` (generated against `MacOSX26.2.sdk`, not regenerated since) counts top-level `CA*.h` symbols only: classes, protocols, typealiases, constants and functions. A symbol is VERIFIED when a named Rust item exists for it, even if only some of its members are wrapped. Class members are not counted; see "Known member gaps" below for the ones that are missing.
...
| `CASpringAnimation` | ✅ | `configure` convenience plus existing spring accessors; `with_perceptual_duration`, `perceptual_duration`, `bounce`, `allows_overdamping` (macOS 14) | `examples/07_ca_spring_animation_configure.rs`, `tests/ca_spring_animation_tests.rs` | |
| `CATransition` | ✅ | `has_subtype`, `clear_subtype`, `filter_name` / `set_filter_name` (Core Image transition filters by name) | `examples/08_ca_transition_subtype.rs`, `tests/ca_transition_tests.rs` | Arbitrary `CIFilter` objects are not accepted, only `kCICategoryTransition` names |
...
| `CATransaction` | ✅ | `lock`, `unlock`, `lock_guard`, animation timing-function helpers, `set_completion_handler` (owned `FnOnce + Send` closure; the raw `set_completion_block` is `unsafe`) | `examples/10_ca_transaction_defaults.rs`, `tests/ca_transaction_tests.rs`, `tests/transaction_completion_tests.rs` | Completion handlers run on the main thread |
...
| `CAMetalLayer` | ✅ | `framebuffer_only`, `maximum_drawable_count` (setter accepts only 2 or 3), `presents_with_transaction`, `display_sync_enabled`, `allows_next_drawable_timeout`, `wants_extended_dynamic_range_content`, `preferred_device`, `developer_hud_properties` (macOS 13) | `examples/12_ca_metal_layer_properties.rs`, `tests/ca_metal_layer_tests.rs` | `residencySet` (macOS 26) is not wrapped: apple-metal's `ResidencySet` has no public constructor |
...
| `CARemoteLayerClient` / `CARemoteLayerServer` / `CARenderer` color space | ✅ | `RemoteLayerClient`, `RemoteLayerServer`, `Renderer::new_with_color_space`, `Renderer::add_update_rect`, `CGColorSpace` | `examples/28_ca_remote_layer_renderer.rs`, `tests/ca_remote_layer_renderer_tests.rs`, `tests/renderer_update_rect_tests.rs` | Keeps the existing renderer constructor intact while adding the color-space option |
...
- `COVERAGE_AUDIT.md` reports `193 / 193` non-exempt top-level QuartzCore `CA*.h` symbols with a named Rust item. It does not count members, so it says nothing about the member gaps below.
- `CAOpenGLLayer` remains intentionally exempt because it is deprecated on macOS.

## Known member gaps

Members of wrapped classes that have no Rust API yet (from a scan of the `MacOSX26.5.sdk` headers against the bridge; not exhaustive for methods):

| Class | Not wrapped |
| --- | --- |
| `CALayer` | `superlayer`, `insertSublayer:*`, `replaceSublayer:with:`, `convertPoint/Rect/Time:*`, `hitTest:`, `containsPoint:`, `presentationLayer`, `modelLayer`, `animationKeys`, `animationForKey:`, `removeAllAnimations`, `affineTransform`, `preferredFrameSize`, `contentsRect`, `contentsCenter`, `contentsHeadroom`, `wantsDynamicContentScaling`, `minificationFilterBias`, `opaque`, `needsDisplayOnBoundsChange`, `drawsAsynchronously`, `allowsEdgeAntialiasing`, `allowsGroupOpacity`, `compositingFilter`, `filters`, `backgroundFilters`, `shouldRasterize`, `rasterizationScale`, `shadowPath`, `style`, `renderInContext:`, `scrollPoint:`, `scrollRectToVisible:` |
| `CAEmitterCell` | `lifetimeRange`, `emissionLatitude`, `velocityRange`, `x/y/zAcceleration`, `scaleRange`, `scaleSpeed`, `spinRange`, color ranges and speeds, `contentsRect`, `minificationFilterBias`, `style` |
| `CAMetalLayer` | `residencySet` |
| `CADisplayLink` | `preferredFramesPerSecond`, `frameInterval` (both deprecated), adding to other run loops or modes |
| `CATransition` | arbitrary `filter` objects (only named Core Image transition filters) |
| `CATransaction` | `valueForKey:` / `setValue:forKey:` |
